...
public class UserAd_RcrtResumeAdsBatchProcess {
	AbstractApplicationContext context = new AnnotationConfigApplicationContext(
			AppConfig.class);
	private static Logger logger = Logger
			.getLogger(UserAd_RcrtResumeAdsBatchProcess.class);
	DB2Service db2Service = (DB2Service) context.getBean("db2Service");

	public static void main(String[] args) throws IOException {
		logger.info("====UserAd_RcrtResumeAdsBatchProcess has been started=====");
		UserAd_RcrtResumeAdsBatchProcess sicbp = new UserAd_RcrtResumeAdsBatchProcess();
		...
		jbps.setProcessName(Constants.USERAD_RCRTRESUME_ADS.getValue());
		sicbp.process(jbps);
		logger.info("====UserAd_RcrtResumeAdsBatchProcess has been finished=====");
	}
